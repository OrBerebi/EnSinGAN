...
def _handle_path_memorization(path, sess, model_name, is_checksize, is_check_png):
    ...
    #x = []
    #for fn in files:
    #  img_py =  img_read_checks(fn, imsize, is_checksize, imsize, is_check_png)
    #  x = [x,img_py]
    #x = np.array(x)
    x = np.array([np.array(img_read_checks(fn, imsize, is_checksize, imsize, is_check_png)) for fn in files])
    #x = [np.array(img_read_checks(fn, imsize, is_checksize, imsize, is_check_png)) for fn in files]
    #for item in x:
    # print()
...
def calculate_kid_given_paths(paths, model_name, model_path, feature_path=None,files_path=None):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        if feature_path is None:
            m2, s2, features2, files2 = _handle_path_memorization(paths[1], sess, model_name, is_checksize = True, is_check_png = True)
        else:
        ...
        #print("saved features shape: " + shape(features2))
        np.save(paths[1]+"/landscapes_features.npy", features2)
...
#user_images_unzipped_path = './Input/Images/monet_jpg_names/'
    images_path = [user_images_unzipped_path,'./Input/Images/landscapes/']

    public_path = './eval_dir/inception_net/classify_image_graph_def.pb'
    feature_path = './eval_dir/monet_features.npy'
    files_path = './eval_dir/monet_files.npy'
    fid_epsilon = 10e-15

    #closest_image,real_image,val  = calculate_kid_given_paths(images_path, 'Inception', public_path,feature_path,files_path)
    closest_image,real_image,val  = calculate_kid_given_paths(images_path, 'Inception', public_path)
