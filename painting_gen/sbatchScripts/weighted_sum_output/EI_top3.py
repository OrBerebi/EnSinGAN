...
for line in Lines:
    ...
    img_path = path + monet_name_1 +"/" + photo_name + name_scale_out
    img = np.asarray(Image.open(img_path))
    img_w_1 = (val_1 * img)/val_sum

    img_path = path + monet_name_2 +"/" + photo_name + name_scale_out
    img = np.asarray(Image.open(img_path))
    img_w_2 = (val_2 * img)/val_sum

    img_path = path + monet_name_3 +"/" + photo_name + name_scale_out
    img = np.asarray(Image.open(img_path))
    img_w_3 = (val_3 * img)/val_sum

    save_path_curr = save_path_curr_dir +"/EI_"+ photo_name + ".jpg"
    img_out = ((img_w_1 + img_w_2 + img_w_3)).astype(np.uint8)
